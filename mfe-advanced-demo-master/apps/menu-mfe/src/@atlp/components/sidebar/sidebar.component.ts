...
export class AtlpSidebarComponent implements OnInit, OnDestroy {
    ...
    /**
     * Update timeOpened - Since it must be unique
     */
    ...
    /**
     * remove z-index for sidebars with superimposed = true
     */
    removeIndex(): void {
        // if the sidebar is not overlay then we shouldn't remove the z-index
        if (!this.superimposed) { return; }
    ...
    /**
     * Add class ${this._atlpConfig.superimposed.class}-${index}
     * @param index - type number
     */
    addClassSuperimposedSidbar(index): void {
        this._renderer.addClass(this._elementRef.nativeElement, `${this._atlpConfig.superimposed.class}-${index}`);
    }
    /**
     * Add class active
     */
    ...
    /**
     * remove class ${this._atlpConfig.superimposed.class}-${index}
     */
    removeClassSuperimposedSidbar(): void {
        this._atlpSidebarService.allOpenedSuperimposedSidebar.forEach((item, index) => {
            this._renderer.removeClass(this._elementRef.nativeElement, `${this._atlpConfig.superimposed.class}-${index}`);
        });
    }
    /**
     * remove class active
